...
class LocalStorageService {
  ...
  static Future<void> setPatientAsActive(Map<String, dynamic> patientJson) async {
    final box = Hive.box<ActivePatient>(_activePatientsBox);
    final patientId = patientJson['id'] as String;
...
static Map<String, dynamic>? getMedicalRecord({
  ...
  static Future<String> exportAllData() async {
    ...
    final exportData = {
      'activePatients': activePatientsBox.values.map((p) => p.toJson()).toList(),
      'medicalRecords': medicalRecordsBox.values.map((v) => jsonDecode(v)).toList(),
